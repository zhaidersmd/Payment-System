...
import com.paymentplatform.payment.dto.*;
import com.paymentplatform.payment.entity.PaymentStatus;
import com.paymentplatform.payment.service.PaymentAnalyticsService;
...
public class PaymentController {
    ...
    public PaymentController(PaymentService paymentService, PaymentStatusCacheService cacheService, PaymentAnalyticsService paymentAnalyticsService) {
    ...
    @Operation(summary = "Create a payment", description = """
            ...
            """)
    public PaymentResponse createPayment(@Parameter(name = "Idempotency-Key", description = "Unique key used to prevent duplicate payment requests", required = true, example = "payment-request-123") @RequestHeader("Idempotency-Key") String idempotencyKey, @Valid @RequestBody CreatePaymentRequest request) {
    ...
    @GetMapping("/customer/{customerId}/batch-summary")
    public PaymentBatchSummary getBatchSummary(@PathVariable String customerId) {
        return paymentAnalyticsService.getBatchSummary(customerId);
    }
